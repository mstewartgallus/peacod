...
@NodeInfo(shortName = "!")
public abstract class ForceActionNode extends ActionNode {
    @Executed
    @Child
    ValueNode thunk;

    ForceActionNode(ValueNode thunk) {
        this.thunk = thunk;
    }

    public static ForceActionNode of(ValueNode thunk) {
        return ForceActionNodeGen.create(thunk);
    }

    @Specialization
    Action<?> force(ThunkValue<?> thunk) {
        return thunk.action;
    }
}
